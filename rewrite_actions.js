...
export async function getContacts() {
  try {
    ...
    if (error) throw error;
    return (data || []).map(doc => ({ id: doc.id, ...doc, order: doc.sort_order }));
  } catch (error) {
...
export async function addSocialMedia(socialData) {
  try {
...
fs.writeFileSync('client/src/app/admin/actions.js', code);
console.log('Successfully written clean actions.js');
